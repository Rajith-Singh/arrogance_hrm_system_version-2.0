...
<div class="row">
    ...
    fetch('/api/employees/total')
        ...
        // Fetch counts for Active Employees and On Leave Employees
        fetch('/api/register/counts')
            .then(response => response.json())
            .then(data => {
                // Update the Active Employees count
                document.querySelector('.card-body .highlight:nth-child(2)').innerText = data.active_employees;

                // Update the On Leave Employees count
                document.querySelector('.card-body .highlight:nth-child(3)').innerText = data.on_leave_employees;
            })
            .catch(error => console.error('Error fetching register counts:', error));
    });
