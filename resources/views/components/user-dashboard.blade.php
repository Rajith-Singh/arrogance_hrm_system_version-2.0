            ...
            <div class="col-md-6">
                <div class="card text-center">
                    <div class="card-body">
                        ...
                        <!-- Pass User ID to the Attendance System -->
                        <a href="https://outstation.arrogance.lk?user_id={{ Auth::user()->id }}&user_name={{ Auth::user()->name }}" 
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($remainingLeaves as $type => $data)
                                    @php
                                        // Default values from the existing data
                                        ...
                                        // Apply special rules only for Annual Leave for fresh permanent employees
                                        if ($type === 'Annual Leave' && auth()->user()->sub_category == 'fresh' && auth()->user()->permanent_date) {
                                            $permanentDate = new DateTime(auth()->user()->permanent_date);
                                            $permanentYear = $permanentDate->format('Y');
                                            $permanentMonth = $permanentDate->format('n');
                                            $currentYear = date('Y');
                                            
                                            // First year (year of becoming permanent) gets 0 annual leaves
                                            if ($currentYear == $permanentYear) {
                                                $allocated = 0;
                                            } 
                                            // Subsequent years follow the prorated allocation
                                            else {
                                                if ($permanentMonth >= 1 && $permanentMonth <= 3) { // Jan-Mar
                                                    $allocated = 14;
                                                } elseif ($permanentMonth >= 4 && $permanentMonth <= 6) { // Apr-Jun
                                                    $allocated = 10;
                                                } elseif ($permanentMonth >= 7 && $permanentMonth <= 9) { // Jul-Sep
                                                    $allocated = 7;
                                                } else { // Oct-Dec
                                                    $allocated = 4;
                                                }
                                            }
                                            // Update remaining leaves calculation
